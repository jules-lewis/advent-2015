...
message asking you to enter a code from the instruction manual. Apparently, 
it refuses to run unless you give it that code. No problem; you'll just look 
up the code in the--
...
first few numbers; for example, you're missing the one at 7,1 
that would come before 6,2. But, it should be enough to let 
your -- oh, it's time for lunch! Bye!" The call disconnects.

Santa looks nervous. Your puzzle input contains the message on 
the machine's console. What code do you give the machine?
...
star sits at the bottom, awaiting its friends. Looks like you need to provide 
49 yourself.
...
'''
So, first we need to work out the ordinal of the code we need. Looking down
the rows in the example, the increment goes up one every time. SO let's work 
out which code is in row 2978 (from the puzzle input), column 1
'''
increment = 1
code = 1
for row in range(2, 2978+1):
    code += increment
    increment += 1

'''
Now we go along the columns, stepping up in the same way
'''
increment = row + 1
