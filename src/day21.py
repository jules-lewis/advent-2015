...
1 damage. So, if the attacker has a damage score of 8, and the defender has 
...
for weapon in w:
    player_one.append([weapon])

for weapon in w:
    for armour in a:
        player_one.append([weapon, armour])

new = deepcopy(player_one)
one_ring = []
for n in new:
    for ring in r:
        set_up = n + [(ring)]
        one_ring.append(set_up)
        player_one.append(set_up)
...
for p in player_one:
    ...
    for i in p:
        ...
        dam += i[1]
        arm += i[2]
    set_ups.append((cst, dam, arm))

def fight(to_win, set_up):

    p1_attack = set_up[1] - BOSS_ARM
    boss_attack = BOSS_DAM - set_up[2]
    p1_hp = PLAYER_HP
    boss_hp = BOSS_HP
...
#Part 1    
for set_up in sorted(set_ups):
    if fight(True, set_up):
        print(set_up[0])
        break

#Part 2
for set_up in sorted(set_ups, reverse=True):
    if fight(False, set_up):
        print(set_up[0])
        break
